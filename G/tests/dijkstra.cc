...
int TYPE = 3;           // 1:continue忘れ, 2:最大ヒープ, 3:最大ヒープ２, 4:負辺
int MAX_N = 2500;        // 頂点数の最大値
int MAX_M = 5000;        // 辺数の最大値
int MIN_C = 0;          // 辺のコストの最小値
int MAX_C = 1e9-1;        // 辺のコストの最大値
int MAX_A = 1e9;        // 辺のコストの最大値
bool DIRECTED = true;  // 有向かどうか
bool V1_TO_Vn = true;  // s=1, t=n とするかどうか
...
struct Graph {
  ...
  void output() {
    ...
    for (Edge& e : es) {
      printf("%d %d %d\n", e.a + INDEXED, e.b + INDEXED, MAX_A - e.cost);
    }
  }
};
